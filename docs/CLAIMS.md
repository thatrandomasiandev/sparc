...
|----|--------------------|------------|--------|
| C-mot-1 | Many distinct unit rewards induce the **same** optimal policy on our domain | E-eq | planned |
| C-mot-2 | Fraction of reward-space volume (or particle pairs) that is decision-irrelevant grows with `d` | E-eq-d | planned |
...
| ID | Claim | Experiment | Status |
|----|-------|------------|--------|
| C-main-1 | VOI final regret **<** random (paired bootstrap CI excludes 0) | E3-power | preliminary / needs re-run |
| C-main-2 | VOI final regret **<** BALD (paired CI excludes 0) | E3-power | **contested** — ablation n=24 NS / favors BALD |
| C-main-3 | True-regret oracle ≪ VOI (oracle gap) | E3-ablate | supported (gap ~1.9) |
| C-main-4 | BALD not significantly better than random at d=8 (legacy claim weakened) | E3-power | preliminary |
...
| ID | Claim | Experiment | Status |
|----|-------|------------|--------|
| C-mech-1 | When ≥ρ fraction of features are **unreachable** (decision-irrelevant), VOI−BALD gap **grows** in ρ | E-decouple | planned — queue #2 |
| C-mech-2 | Population prior beats uniform particle prior on final regret (same budget) | E-prior | planned |
| C-mech-3 | Misspecified prior (train modes ≠ test modes) shrinks or flips VOI advantage | E-prior-shift | planned |
| C-mech-4 | Acting with minimax-regret / sampled particle policy reduces oracle gap vs mean action | E-act | planned |
...
| C-mech-7 | Negative control: labels from reward orthogonal to task ⇒ VOI ≉ better than random | E-neg | planned |
...
| C-mech-10 | Model mismatch (satisficing / lex / fatigue): graceful degradation table | E-mismatch | planned |

### A6. Abstract sentences that currently overreach
...
|-----------------|-------|--------|
| “lower final policy regret than random and BALD” (150 runs) | C-main-1, C-main-2 | Re-run E3-power with true-regret oracle; demote BALD clause if NS |
| “large gap to an oracle” | C-main-3 | Keep; cite E3-ablate |
| “population prior makes criterion usable” | C-mech-2 | Must run E-prior before this sentence stays |
...
- Offline human clip study (hardware still parked for live robots)
